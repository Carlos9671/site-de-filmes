    ...
    <div class="fixed top-0 left-0 right-0 z-10" x-data="{ menuAberto: false }">
        ...
        <nav class="bg-slate-900 border-b border-slate-700 px-6 py-4 flex items-center justify-between">
            ...
            <div class="hidden md:flex items-center gap-6 text-sm text-slate-300">
                ...
                <a href="/generos" class="hover:text-violet-400 transition">Editar Gêneros</a>
        ...
        <div x-show="buscarAberto" class="bg-slate-900 border-b border-slate-700 px-6 py-3">
            <form action="/admin" method="GET" class="flex gap-3">
    ...
    <div x-show="sidebarAberta" class="md:hidden fixed top-16 left-0 right-0 z-20 bg-slate-900 border-b border-slate-700 px-6 py-4">
        <ul class="flex flex-wrap gap-2">
            <li>
                ...
                <a href="/admin" @click="sidebarAberta = false" class="block px-3 py-1 rounded-full text-sm transition {{ $classTodos }}">
                    Todos
                </a>
            </li>
            @foreach($generos as $genero)
                ...
                <li>
                    <a href="/admin?genero={{ urlencode($genero->genero) }}" @click="sidebarAberta = false" class="block px-3 py-1 rounded-full text-sm transition {{ $classGenero }}">
                        {{ $genero->genero }}
                    </a>
                </li>
            @endforeach
    ...
    <div class="flex pt-16">
        ...
        <aside class="hidden md:block w-52 min-h-screen bg-slate-900 border-r border-slate-700 p-6 fixed top-16 left-0">
            ...
            <ul class="flex flex-col gap-2">
                <li>
                    ...
                    <a href="/admin" class="block w-full text-left px-3 py-2 rounded-lg text-sm transition {{ $classTodos }}">
                        Todos
                    </a>
                </li>
                @foreach($generos as $genero)
                    ...
                    <li>
                        <a href="/admin?genero={{ urlencode($genero->genero) }}" class="block w-full text-left px-3 py-2 rounded-lg text-sm transition {{ $classGenero }}">
                            {{ $genero->genero }}
                        </a>
                    </li>
                @endforeach
        ...
        <main class="w-full md:ml-52 flex-1 p-4 md:p-8 flex flex-col gap-4">
            ...
            @foreach ($filmes as $filme)
                <div class="bg-slate-900 border border-slate-700 rounded-xl flex flex-col md:flex-row gap-4 p-4 hover:border-violet-500 transition">
                    ...
                    <div class="flex flex-col justify-center gap-2 flex-1">
                        ...
                        <div class="flex gap-2 mt-2">
                            <a href="{{ route('filmes.edit', $filme->id) }}"
                                class="bg-blue-500 hover:bg-blue-400 px-3 py-1 rounded text-xs transition">
                                ✏️ Editar
                            </a>
                            <button onclick="deletar({{ $filme->id }})"
                                class="bg-red-600 hover:bg-red-500 px-3 py-1 rounded text-xs transition">
                                🗑 Excluir
                            </button>
                        </div>
                    </div>
